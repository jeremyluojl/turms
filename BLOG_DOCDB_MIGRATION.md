...
**测试方法：** EC2（VPC 内）通过 Motor（asyncio 异步 pymongo）直连 DocumentDB writer，消除公网 RTT 和应用层开销。每场景 **5,000 次请求**（p99 有 50 个样本，统计稳定），正式计时前执行 warmup。读操作覆盖用户主键查询、用户关系列表（`oid` 索引）、群成员列表（`gid` 索引）三类，随机选取。写操作为 insert + delete 单条 message，保持数据集大小不变。实例规格：DocumentDB **`db.r8g.large`** × 2（1 writer + 1 reader）。

### 5.1 并发梯度（纯读）

| 并发数 | RPS | p50 | p95 | p99 |
|---|---|---|---|---|
| 1 | 316 | 2.3 ms | 7.1 ms | 12.6 ms |
| 10 | 1,072 | 7.9 ms | 18.3 ms | 36.8 ms |
| **50（甜点区）** | **1,341** | **35.9 ms** | **58.6 ms** | **70.6 ms** |
| 100 | 1,306 | 74.9 ms | 100.2 ms | 116.9 ms |

吞吐峰值约 **1,341 RPS**，concurrency=50 达到饱和，100 无继续增长。
...
1. **hashed → range 降级无性能损失**：涉及降级索引的集合（群组、群成员等）p50 均在 1.4–1.5 ms，与主键查询相当，证明 range(1) 索引对等值查询同样高效。

2. **r8g.large 相比 t3.medium 提升显著**：concurrency=50 时 RPS 从 ~169 提升至 ~1,341（**7.9 倍**），p99 从 2,879 ms 降至 70.6 ms（**41 倍**），适合并发超过 20 的生产场景。

3. **甜点区：concurrency=10**，RPS=1,072，p99=36.8 ms，是 r8g.large 性价比最高的工作点。

4. **功能验证全部通过**：Admin API 测试 20/20，WebSocket 端到端测试 33/33，覆盖消息收发、群组管理、好友关系、实时推送等完整业务链路。
